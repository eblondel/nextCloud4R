...
#' }
#' 
#' @note Logger class used internally by nextCloud4R
#'
nextCloud4RLogger <-  R6Class("nextCloud4RLogger",
  public = list(
    #logger
    verbose.info = FALSE,
    verbose.debug = FALSE,
      ...
      if(!missing(logger)){
        if(!is.null(logger)){
          ...
          if(!(self$loggerType %in% c("INFO","DEBUG"))){
            stop(sprintf("Unknnext logger type '%s", logger))
          }
